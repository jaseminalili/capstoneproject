...
exports.deleteAccount = async (req, res, next) => {
  ...
  try {
    ...
    // Set reporter to NULL — preserves team tasks instead of deleting them
    await client.query(
      'UPDATE tasks SET reporter_id = NULL WHERE reporter_id = $1',
      [req.user.id]
    )
 
    // CASCADE will handle: workspaces (if owner), workspace_members,
    // project_members, task assignments, comments, notifications
