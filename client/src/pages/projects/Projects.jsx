...
function FilterDropdown({ label, options, value, onChange }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute top-full mt-1.5 left-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl py-1.5 z-30 min-w-full" onMouseLeave={() => setOpen(false)}>
          <button onClick={() => { onChange(''); setOpen(false) }} className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 ${!value ? 'text-blue-600 font-semibold' : 'text-gray-700 dark:text-gray-300'}`}>{label}</button>
          {options.map(o => (
            <button key={o.v} onClick={() => { onChange(o.v); setOpen(false) }} className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 ${value === o.v ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 font-semibold' : 'text-gray-700 dark:text-gray-300'}`}>{o.l}</button>
          ))}
        </div>
      )}
    </div>
          ...
          {members.map(m => {
            ...
            return (
              <button key={m.id} type="button" onClick={() => toggle(m.id)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${sel ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-gray-300 dark:hover:border-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
    ...
    <div className="p-6 sm:p-8 max-w-7xl mx-auto">
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and track your workspace projects</p>
        </div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects…"
            className="pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm bg-white dark:bg-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 w-56" />
        </div>
        ...
        {(sf || pf || search) && (
          <button onClick={() => { setSf(''); setPf(''); setSearch('') }} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 underline">Clear filters</button>
        )}
      </div>
            ...
            <div key={p.id} onClick={() => navigate(`/projects/${p.id}`)}
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0 mr-2">
                  <h3 className="font-bold text-gray-900 dark:text-white text-sm leading-tight truncate">{p.name}</h3>
                </div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mb-3 leading-relaxed min-h-[2rem]">{p.description || 'No description provided.'}</p>
              ...
              <div className="flex items-center justify-between">
                ...
                {p.end_date && (
                  <span className="text-xs text-gray-400 dark:text-gray-500">{new Date(p.end_date).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}</span>
                )}
              </div>
            </div>
          ))}
