...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Total Projects',  value: projects.length, icon: FolderKanban, bg: 'bg-blue-50',    txt: 'text-blue-600',   border: 'border-blue-100' },
    { label: 'Active Projects', value: activeProjects,  icon: TrendingUp,   bg: 'bg-emerald-50', txt: 'text-emerald-600',border: 'border-emerald-100' },
    { label: 'Team Members',    value: team.length,     icon: Users,        bg: 'bg-violet-50',  txt: 'text-violet-600', border: 'border-violet-100' },
    { label: 'Open Tasks',      value: myTasks.length,  icon: CheckSquare,  bg: 'bg-amber-50',   txt: 'text-amber-600',  border: 'border-amber-100' },
  ]
  ...
  return (
    <div className="p-6 sm:p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1 text-sm">{ws?.name} · {new Date().toLocaleDateString('en-GB', { weekday:'long', day:'2-digit', month:'long', year:'numeric' })}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              {projects.slice(0, 5).map(p => (
                <div key={p.id} onClick={() => navigate(`/projects/${p.id}`)}
                  className="flex items-center gap-4 p-3.5 rounded-xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 cursor-pointer transition-all group">
                  ...
                  <ArrowRight size={15} className="text-gray-300 group-hover:text-blue-400 transition-colors shrink-0" />
                </div>
              ))}
            </div>
          )}
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-xs text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full font-medium">{myTasks.length} open</span>
          </div>
            ...
            <div className="space-y-2">
              {myTasks.slice(0, 8).map(t => (
                <button key={t.id} onClick={() => navigate(`/task/${t.id}?projectId=${t.project_id}`)}
                  className="w-full flex items-start gap-3 p-3 rounded-xl hover:bg-gray-50 text-left transition-colors group border border-transparent hover:border-gray-200">
                  <div className="w-2.5 h-2.5 rounded-full mt-1.5 shrink-0" style={{ background: PRIORITY_COLORS[t.priority] || '#9CA3AF' }} />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-800 truncate group-hover:text-gray-900">{t.title}</p>
        ...
        <div className="card p-6 mt-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {team.slice(0, 10).map(m => (
              <div key={m.id} className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 rounded-full border border-gray-100">
