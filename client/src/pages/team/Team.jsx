...
function InviteModal({ ws, onClose }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Role <span className="text-red-500">*</span></label>
      ...
      <div className={`rounded-xl px-4 py-3 text-xs font-medium leading-relaxed ${
        role === 'admin'
          ? 'bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 text-purple-700 dark:text-purple-300'
      ...
      {inviteSuccess && (
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl px-4 py-3 text-sm text-emerald-700 dark:text-emerald-400">
          ...
          <p className="text-xs text-emerald-600 dark:text-emerald-500">
            A professional email has been delivered to <strong>{email}</strong>.
          </p>
        </div>
      )}
    ...
    <div className="p-6 sm:p-8 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage workspace members and their access roles</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          { label: 'Total Members',   value: members.length, icon: Users,       bg: 'bg-blue-50 dark:bg-blue-900/30',    txt: 'text-blue-600',    border: 'border-blue-100 dark:border-blue-800'    },
          { label: 'Active Projects', value: activeProjects, icon: Activity,    bg: 'bg-emerald-50 dark:bg-emerald-900/30', txt: 'text-emerald-600', border: 'border-emerald-100 dark:border-emerald-800' },
          { label: 'Total Projects',  value: projects.length,icon: CheckSquare, bg: 'bg-violet-50 dark:bg-violet-900/30',  txt: 'text-violet-600',  border: 'border-violet-100 dark:border-violet-800'  },
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100 dark:border-gray-700 bg-gray-50/70 dark:bg-gray-700/50">
              <th className="text-left text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3.5">Name</th>
              <th className="text-left text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3.5 hidden sm:table-cell">Email</th>
              <th className="text-left text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-6 py-3.5">Role</th>
          ...
          <tbody>
            ...
            {!loading && visible.map(m => (
              <tr key={m.id} className="border-b border-gray-50 dark:border-gray-700 last:border-0 hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 hidden sm:table-cell">{m.email}</td>
                <td className="px-6 py-4"><RoleBadge value={m.role} /></td>
                {myRole && myRole !== 'member' && (
                  <td className="px-6 py-4">
                    {m.id !== me?.id && (
                      <div className="flex items-center gap-1.5 justify-end">
                        <button onClick={() => handleRoleToggle(m.id, m.role)}
                          title={`Make ${m.role === 'admin' ? 'member' : 'admin'}`}
                          className="p-1.5 hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg text-gray-400 hover:text-purple-600 transition-colors">
                          <Crown size={14} />
                        </button>
      ...
      <Modal isOpen={showInvite} onClose={() => { setShowInvite(false); dispatch(clearInviteStatus()) }}
        title="Invite Team Member" subtitle={`Inviting to: ${ws?.name}`}>
        <InviteModal ws={ws} onClose={() => { setShowInvite(false); dispatch(clearInviteStatus()) }} />
      </Modal>
 
      <ConfirmDialog isOpen={!!removeId} onClose={() => setRemoveId(null)} onConfirm={handleRemove}
        title="Remove Team Member"
        message="This will remove the member from the workspace. They will lose access to all projects."
